...
import { Dialog, DialogContent, DialogTitle, Typography, Chip, Box } from '@mui/material'
...
function MakeCocktail() {
    ...
    const handleIngrChange = (e) =>{
        ...
        const isSelected = selectedIngredients.includes(newIngredient)

        if (isSelected){
            setSelectedIngredients(selectedIngredients.filter(ingredient => ingredient !== newIngredient))
        }else {
            setSelectedIngredients([...selectedIngredients, newIngredient]) //spread
        }
    ...
    const handleSubmit = async (event) =>{
        ...
        try {
           ...
           const response = await axios.post(`${import.meta.env.VITE_URL_BACKEND}/cocktails`,newCocktail, {headers: {
        ...
        <form onSubmit={handleSubmit}>
            ...
            <label>
                ...
                >
                    ...
                    <option value="Sweet and Sour Cocktails">Sweet and Sour Cocktails</option>
            ...
            <label>
                ...
                {ingredientsValue.length > 0 && (
                    <Chip label={ingredientsValue.map((ing) => ing.name).join(', ')} />//TODO A VER ESTO
                )}
                <Dialog open={openDialog} onClose={handleCloseDialog}>
                <button onClick={handleAcceptIngredients}>Accept Ingredients</button>
